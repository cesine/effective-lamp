...
test.describe('cryptogram tests', () => {
  test('should send game events to google analytics', async ({ page }) => {
    ...
    // Click on the alphabet does nothing
    await page.getByLabel('A', { exact: true }).click()

    // Enter an incorrect guess
    await page.getByRole('button', { name: 'Q' }).click()
    await page.keyboard.type('o')
    expect(await page.getByRole('button', { name: 'Q' })).toHaveText('O')

    // Enter a correct guess
    await page.getByRole('button', { name: 'Q' }).click()
    await page.keyboard.type('i')
    expect(await page.getByRole('button', { name: 'Q' })).toHaveText('I')
    ...
    expect(gameEvents[13]).toMatch(/level_end: Cryptogram \d+ hi/)
